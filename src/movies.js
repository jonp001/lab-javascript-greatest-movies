...
function scoresAverage(moviesArray) {
  ...
  if (movies.length === 0) {
    ...
    let avgScore = sum / movies.length;
...
function dramaMoviesScore(moviesArray) {
  if (movies.length === 0) {
    ...
    let dramaMovies = moviesArray.filter(({ genre }) => genre == "Drama"),
      dramaAvg =
        dramaMovies.reduce((total, next) => total + next.score, 0) /
        dramaMovies.length;
...
function orderByYear(moviesArray) {
  let sortedMovies = [...moviesArray].sort((a, b) => {
    if (a.year > b.year) {
      return 1;
...
function orderAlphabetically(moviesArray) {
  let alphabeticalOrder = [...moviesArray].sort((x, y) => {
    if (x.title === y.title) {
      return 0;
